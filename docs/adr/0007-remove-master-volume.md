...
- `metronome.js` keeps its master `GainNode`. It is not the removed control but the stage `stop()` zeroes to silence the graph, and it is now fixed at unity through the `MASTER_GAIN` constant. The device owns output loudness, so this stage does not attenuate the per-Rhythm level before it reaches the system control.
