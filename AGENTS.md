...
`tsconfig.json` covers the source tree and `scripts/`; `test/` and `e2e/` are parsed but not checked, which is where the ratchet goes next. That gap predates TypeScript being the source language and was not widened by it.
