...
- `biome.jsonc`: lint and formatter configuration. `.jsonc` rather than `.json` because strict JSON cannot hold the paragraph explaining the one disabled rule, and a rule turned off without a reason is one nobody can safely turn back on. What gets linted is decided in `scripts/lint.ts` rather than here — see the Verification section — so the exclusions in this file now only bound a `biome check .` run by hand.
...
Both of those are `scripts/lint.ts`, which hands Biome the files git tracks rather than the repository directory to scan. `biome check .` reported a different count from one run to the next here — 51 files, or 115, or something in between — and from a checkout under `.claude/worktrees/` it reported none at all, because the exclusion that stops the primary checkout linting copies of itself matched the path such a checkout is at, so `.` was refused and the whole tree with it. The second failure is loud and the first is not, and the silent one is the worse of the two: a green result over a subset Biome chose by itself says nothing about what went unread. An explicit list has one answer, leaves the untracked worktree checkouts out by construction rather than by a rule that has to be written correctly, and enrols a new file when it is committed — the argument `test/syntax.test.ts` already makes for its own enumeration. `test/lint.test.ts` holds it from both places, and asserts the files Biome reported it opened rather than only that nothing was wrong, since those two read identically when the answer is none. What it holds that list against is a second enumeration of its own, run through Biome by a route that is not `scripts/lint.ts`, because naming a few files it must have opened bounds the set at a few files: an enumeration quietly shrunk to fifteen of the fifty-three, losing `e2e/`, `test/`, `scripts/` and `types/` whole, satisfies every such assertion and passes. Two enumerations obliged to agree have no such blind spot, and neither side keeps a list of the extensions Biome understands — it skips what it cannot read, on both sides, so nothing here goes stale the day it learns another. The worktree fixture also carries a file whose name contains a tab, which is the input that separates reading `git ls-files -z --stage` from reading it nearly right: `-z` leaves an interior tab unescaped, so splitting an entry on every tab rather than on its first truncates the pathname, and Biome answers a name that does not exist with a diagnostic and an exit status of zero.
